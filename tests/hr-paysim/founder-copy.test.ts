...
test("locks the Product Engineer supporting copy, prompt, and title formatter", () => {
  ...
  assert.match(FOUNDER_COPY["non_claim.higher_salary"], /적정 연봉을 계산하거나.*높은 직원.*잘못됐다고 판단한 결과가 아닙니다/);
  assert.equal(
...
test("keeps the career-based observed trend and removes the fixed guide", () => {
  assert.equal(formatObservedTrendLabel(6), "현재 자료의 관찰 추세 · 직원 6명");
  assert.equal(
    formatObservedTrendSummary({ employeeCount: 6, direction: "decreasing" }),
    "현재 입력한 직원 6명의 점을 한 줄로 요약하면, 관련 경력이 늘어나는 쪽에서 기본 연봉이 낮아지는 방향입니다. 이 자료만으로 원인이나 적정 연봉을 판단할 수는 없습니다.",
  );
  assert.equal(
    formatObservedTrendSummary({ employeeCount: 3, direction: "increasing" }),
    "현재 입력한 직원 3명의 점을 한 줄로 요약하면, 관련 경력이 늘어나는 쪽에서 기본 연봉도 높아지는 방향입니다. 이 자료만으로 원인이나 적정 연봉을 판단할 수는 없습니다.",
  );
  assert.equal(
    formatObservedTrendSummary({ employeeCount: 4, direction: "flat" }),
    "현재 입력한 직원 4명의 점을 한 줄로 요약하면, 관련 경력에 따라 기본 연봉이 높아지거나 낮아지는 뚜렷한 방향이 없습니다. 이 자료만으로 원인이나 적정 연봉을 판단할 수는 없습니다.",
  );
  assert.equal(
